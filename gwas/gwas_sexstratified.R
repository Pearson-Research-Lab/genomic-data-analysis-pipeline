...
cohort_files <- list.files(cfg_a$sex_stratified_dir, pattern = "\\.rds$", full.names = TRUE)
...
for (file in cohort_files) {
  ...
  for (o in cfg$outcomes$continuous_no_baseline) {
    f <- as.formula(paste0("pheno", o, " ~ Age + Binary.Insulin + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10"))
    run_gwas(pheno, name, o, f, gaussian())
  }

  for (o in cfg$outcomes$continuous_with_baseline) {
    f <- as.formula(paste0("pheno", o, " ~ BL_pheno_", o, " + Age + Binary.Insulin + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10"))
    run_gwas(pheno, name, o, f, gaussian())
  }

  for (o in cfg$outcomes$binary) {
    f <- as.formula(paste0("pheno_", o, " ~ Age + Binary.Insulin + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10"))
    run_gwas(pheno, name, o, f, binomial())
  }
}

cat("GWAS (sex-stratified) complete\n")
