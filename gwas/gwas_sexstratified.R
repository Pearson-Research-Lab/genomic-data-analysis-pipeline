...
build_formula_continuous_no_baseline <- function(pheno_col, pc_count = 10) {
  rhs <- paste(c("Age", "Binary.Insulin", build_pc_terms(pc_count)), collapse = " + ")
  as.formula(paste(pheno_col, "~", rhs))
}

build_formula_continuous_with_baseline <- function(pheno_col, baseline_col, pc_count = 10) {
  rhs <- paste(c(baseline_col, "Age", "Binary.Insulin", build_pc_terms(pc_count)), collapse = " + ")
  as.formula(paste(pheno_col, "~", rhs))
}

build_formula_binary <- function(pheno_col, pc_count = 10) {
  rhs <- paste(c("Age", "Binary.Insulin", build_pc_terms(pc_count)), collapse = " + ")
  as.formula(paste(pheno_col, "~", rhs))
}
...
for (cohort in analysis_cfg$cohorts_sex) {
  ...
  for (outcome_id in names(cfg$outcomes$continuous_no_baseline)) {
    ...
    required_cols <- c(pheno_col, "Age", "Binary.Insulin", paste0("PC", seq_len(pc_count)), "ID")
    check_columns_exist(pheno, required_cols, cohort_label)

    fit <- glmmkin(
      build_formula_continuous_no_baseline(pheno_col, pc_count = pc_count),
  ...
  for (outcome_id in names(cfg$outcomes$continuous_with_baseline)) {
    ...
    required_cols <- c(pheno_col, baseline_col, "Age", "Binary.Insulin", paste0("PC", seq_len(pc_count)), "ID")
    check_columns_exist(pheno, required_cols, cohort_label)

    fit <- glmmkin(
      build_formula_continuous_with_baseline(pheno_col, baseline_col, pc_count = pc_count),
      data = pheno, kins = grm, id = "ID", verbose = FALSE,
      family = gaussian(link = "identity")
    )
  ...
  for (outcome_id in names(cfg$outcomes$binary)) {
    ...
    fit <- glmmkin(
      build_formula_binary(pheno_col, pc_count = pc_count),
      data = pheno, kins = grm, id = "ID", verbose = FALSE,
...
message("Sex-stratified GWAS complete for ancestry: ", ancestry_name)
